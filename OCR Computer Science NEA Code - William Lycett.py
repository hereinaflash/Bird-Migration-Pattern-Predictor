...
all_windows = ["Home", "Past Bird Data", "Past Weather Data", "Predict", "Bird Data View", "Weather Data View", "Help", "Credits"]
...
# Function to create the menu bar on each GUI window
def define_menu_bar(window, windows_to_do):
  ...
  # Iterate through each function to be added to the menu bar
  for item in windows_to_do:
    # Checks if the current function is the same as the current window
    if window.title() == f"Bird Migration Pattern Predictor - {item}":
      # Apply a text object with no command to the menu bar
      menubar.add_command(label=str(item.upper()),
                          font=(str(main_font), 10),
                          activebackground=GREY_GREEN)
    # Checks each function to add it to the menu bar with the appropriate command
    elif item == "Home":
      menubar.add_command(label=str(item),
                          font=(str(main_font), 10),
                          activebackground=OCEAN_GREEN,
                          command=lambda: home_page(root, windows_to_do))
    elif item == "Predict":
      menubar.add_command(label=str(item),
                        font=(str(main_font), 10),
                        activebackground=OCEAN_GREEN,
                        command=lambda: predict_page(root, windows_to_do))
    elif item == "Past Bird Data":
      menubar.add_command(label=str(item),
                        font=(str(main_font), 10),
                        activebackground=OCEAN_GREEN,
                        command=lambda: historical_bird_page(root, windows_to_do))
    elif item == "Past Weather Data":
      menubar.add_command(label=str(item),
                        font=(str(main_font), 10),
                        activebackground=OCEAN_GREEN,
                        command=lambda: historical_weather_page(root, windows_to_do))
    elif item == "Bird Data View":
      menubar.add_command(label=str(item),
                        font=(str(main_font), 10),
                        activebackground=OCEAN_GREEN,
                        command=lambda: bird_data_view_page(root, windows_to_do))
    elif item == "Weather Data View":
      menubar.add_command(label=str(item),
                        font=(str(main_font), 10),
                        activebackground=OCEAN_GREEN,
                        command=lambda: weather_data_view_page(root, windows_to_do))
    elif item == "Help":
      menubar.add_command(label=str(item),
                        font=(str(main_font), 10),
                        activebackground=OCEAN_GREEN,
                        command=lambda: help_page(root, windows_to_do))
    elif item == "Credits":
      menubar.add_command(label=str(item),
                        font=(str(main_font), 10),
                        activebackground=OCEAN_GREEN,
                        command=lambda: credits_page(root, windows_to_do))

# Function to remove all widgets from a window
def clear_window(window):
  # Iterate through each widget in the window and destroy it
  for widget in window.winfo_children():
    widget.destroy()

# Function to create the home page
def home_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Home")
  
  # Call the function to add the menu bar to the window
  define_menu_bar(window, all_windows)

  # Create a text label to greet the user
  home_label_1 = tk.Label(window, text="""Welcome to the Bird Migration Pattern Predictor!
  ...
  home_label_2 = tk.Label(window, image=HOME_PHOTO, width=300, height=210)
  home_label_2.pack()
  # Create a text label to encourage the user to continue
  home_label_3 = tk.Label(window, text="""
...
# Function to create the initial predict page
def predict_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Predict")

  # Call the function to add the menu bar to the window
  define_menu_bar(window, all_windows)

  # Create a text label to direct the user
  predict_label_1 = tk.Label(window, text="Please fill in the fields below to make your prediction:", font=main_font, bg=LIGHT_GREEN)
  predict_label_1.pack()
  # Create the drop-down / check boxes for the user to input their choices
  predict_label_2 = tk.Label(window, text="\nSpecies Name:", font=main_font, bg=LIGHT_GREEN)
  ...
  predict_entry_1 = tk.OptionMenu(window, user_species_name, *species_options)
  predict_entry_1.pack()
  
  predict_label_3 = tk.Label(window, text="\nLocation Name:", font=main_font, bg=LIGHT_GREEN)
  predict_label_3.pack()
  user_location_name = tk.StringVar()
  location_options = ["England", "Wales", "Scotland", "Northern Ireland"]
  predict_entry_2 = tk.OptionMenu(window, user_location_name, *location_options)
  predict_entry_2.pack()

  predict_label_4 = tk.Label(window, text="\nWeather Type:", font=main_font, bg=LIGHT_GREEN)
  predict_label_4.pack()
  use_temp = tk.BooleanVar()
  predict_choice_1 = tk.Checkbutton(window, text="Temperature", font=(str(main_font), 10), variable=use_temp, bg = LIGHT_GREEN)
  predict_choice_1.pack()
  use_wind = tk.BooleanVar()
  predict_choice_2 = tk.Checkbutton(window, text="Wind Speed", font=(str(main_font), 10), variable=use_wind, bg = LIGHT_GREEN)
  predict_choice_2.pack()

  predict_label_5 = tk.Label(window, text="\nPercentage Weather Change (Optional):", font=main_font, bg=LIGHT_GREEN)
  predict_label_5.pack()
  user_weather_change = tk.DoubleVar()
  predict_entry_3 = tk.Entry(window, textvariable=user_weather_change)
  predict_entry_3.pack()

  # Create a button to submit the user's inputs
  predict_button = tk.Button(window, text="Submit", font=main_font, bg=MID_GREEN, activebackground=OCEAN_GREEN, command=lambda: check_prediction_variables(user_species_name, user_location_name, use_temp, use_wind, user_weather_change, window, all_windows, predict_label_1))
  ...
  help_button = tk.Button(window, text="Help", font=main_font, bg=BLUE_GREEN, activebackground=OCEAN_GREEN, command=lambda: tip_box(window, all_windows, tip_text))
  help_button.pack()

# Function to briefly explain the function of a window to the user
def tip_box(window, all_windows, tip_text):
  ...
  tip_button_1 = tk.Button(tip, text="Need More Help?", command=lambda: help_page(window, all_windows))
...
def check_prediction_variables(species, location, use_temp, use_wind, weather_change, window, all_windows, predict_label_1):
  ...
  try:
    ...
    validate_inputs(species, location, use_temp, use_wind, weather_change, window, predict_label_1)
    return
  if (species != "") and (location != "") and (use_temp or use_wind) and (weather_change <= 40 and weather_change >= -40):
    ...
    make_prediction(species, location, weather_type, weather_change, window, all_windows)
  else:
    validate_inputs(species, location, use_temp, use_wind, weather_change, window, predict_label_1)
    return

# Function to find what is wrong with the user's erroneous inputs
def validate_inputs(species, location, use_temp, use_wind, weather_change, window, error_label):
...
def make_prediction(species, location, weather_type, weather_change, window, all_windows):
...
def notify_user(window, all_windows, type):
  ...
  if type == "bird":
    notify_button_1 = tk.Button(notify, text="Take Me There", command=lambda: bird_data_view_page(window, all_windows))
  else:
    notify_button_1 = tk.Button(notify, text="Take Me There", command=lambda: weather_data_view_page(window, all_windows))
...
def bird_data_view_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Bird Data View")

  # Call the function to add the menu bar to the window
  define_menu_bar(window, all_windows)

  # Create a label that explains the function of the page
  view_label = tk.Label(window, text="Choose the category you want to view:", font=main_font, bg=LIGHT_GREEN)
  view_label.pack()

  # Create a frame for the buttons to make them appear side by side
  button_frame = tk.Frame(window, bg=LIGHT_GREEN)
  button_frame.pack()
  ...
  view_image = tk.Label(window, image=data_image, width=210, height=260)
  ...
  view_button_7 = tk.Button(window, text="Enlarge", font=(main_font, 10), bg=MID_GREEN, activebackground=OCEAN_GREEN, command=lambda: enlarge_image("recent_img.png"))
  view_button_7.pack()
...
def weather_data_view_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Weather Data View")
...
def historical_bird_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Past Bird Data")

  # Call the function to add the menu bar to the window
  define_menu_bar(window, all_windows)

  # Create a text label to direct the user
  bird_label_1 = tk.Label(window, text="Please fill in the fields below to display historical bird data:", font=main_font, bg=LIGHT_GREEN)
  bird_label_1.pack()

  # Create the drop-down / check boxes for the user to input their choices
  bird_label_2 = tk.Label(window, text="\nSpecies Name:", font=main_font, bg=LIGHT_GREEN)
  bird_label_2.pack()
  user_species_name = tk.StringVar()
  species_options = ["Arctic Tern", "Cuckoo", "Fieldfare", "House Martin", "Redwing", "Sand Martin", "Swallow", "Swift"]
  bird_entry_1 = tk.OptionMenu(window, user_species_name, *species_options)
  bird_entry_1.pack()
  
  bird_label_3 = tk.Label(window, text="\nLocation Name:", font=main_font, bg=LIGHT_GREEN)
  bird_label_3.pack()
  user_location_name = tk.StringVar()
  location_options = ["England", "Wales", "Scotland", "Northern Ireland"]
  bird_entry_2 = tk.OptionMenu(window, user_location_name, *location_options)
  bird_entry_2.pack()

  bird_label_4 = tk.Label(window, text="\nFirst Year:", font=main_font, bg=LIGHT_GREEN)
  bird_label_4.pack()
  user_first_year = tk.IntVar()
  bird_entry_3 = tk.OptionMenu(window, user_first_year, *all_years[:-1])
  bird_entry_3.pack()

  bird_label_5 = tk.Label(window, text="\nLast Year:", font=main_font, bg=LIGHT_GREEN)
  bird_label_5.pack()
  user_last_year = tk.IntVar()
  bird_entry_4 = tk.OptionMenu(window, user_last_year, *all_years[:-1])
  bird_entry_4.pack()

  # Create a button to submit the user's inputs
  bird_button = tk.Button(window, text="Submit", font=main_font, bg=MID_GREEN, activebackground=OCEAN_GREEN, command=lambda: call_bird_graph(user_species_name, user_location_name, user_first_year, user_last_year, window, all_windows, bird_label_1))
...
def call_bird_graph(species, location, first_year, last_year, window, all_windows, error_label):
...
def historical_weather_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Past Weather Data")

  # Call the function to add the menu bar to the window
  define_menu_bar(window, all_windows)

  # Create a text label to direct the user
  weather_label_1 = tk.Label(window, text="Please fill in the fields below to display historical weather data:", font=main_font, bg=LIGHT_GREEN)
  weather_label_1.pack()

  # Create the drop-down / check boxes for the user to input their choices
  weather_label_2 = tk.Label(window, text="\nWeather Type:", font=main_font, bg=LIGHT_GREEN)
  weather_label_2.pack()
  user_weather_type = tk.StringVar()
  weather_options = ["Temperature", "Wind Speed"]
  weather_entry_1 = tk.OptionMenu(window, user_weather_type, *weather_options)
  weather_entry_1.pack()
  
  weather_label_3 = tk.Label(window, text="\nLocation Name:", font=main_font, bg=LIGHT_GREEN)
  weather_label_3.pack()
  user_location_name = tk.StringVar()
  location_options = ["England", "Wales", "Scotland", "Northern Ireland"]
  weather_entry_2 = tk.OptionMenu(window, user_location_name, *location_options)
  weather_entry_2.pack()

  weather_label_4 = tk.Label(window, text="\nFirst Year:", font=main_font, bg=LIGHT_GREEN)
  weather_label_4.pack()
  user_first_year = tk.IntVar()
  weather_entry_3 = tk.OptionMenu(window, user_first_year, *all_years[:-1])
  weather_entry_3.pack()

  weather_label_5 = tk.Label(window, text="\nLast Year:", font=main_font, bg=LIGHT_GREEN)
  weather_label_5.pack()
  user_last_year = tk.IntVar()
  weather_entry_4 = tk.OptionMenu(window, user_last_year, *all_years[:-1])
  weather_entry_4.pack()

  # Create a button to submit the user's inputs
  weather_button = tk.Button(window, text="Submit", font=main_font, bg=MID_GREEN, activebackground=OCEAN_GREEN, command=lambda: call_weather_graph(user_weather_type, user_location_name, user_first_year, user_last_year, window, all_windows, weather_label_1))
...
# Function to call the graph creation subroutine
def call_weather_graph(weather_type, location, first_year, last_year, window, all_windows, error_label):
  # Get the values of the variable inputted
  f_year = first_year.get()
  l_year = last_year.get()
  ...
  if len(str(f_year)) == 0 or len(str(l_year)) == 0 or len(weather_type) == 0 or len(location) == 0:
    ...
    with open("location.png", "wb") as f_output:
      if len(str(temp)) > 0:
        f_output.write(temp)
    # Alert the user the graphs have been made
    notify_user(window, all_windows, "weather")

# Function to create the page used to explain the program's functions to the user
def help_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Help")

  # Call the function to add the menu bar to the window
  define_menu_bar(window, all_windows)

  # Create a text label to direct the user
  help_label_1 = tk.Label(window, text="Here you can find a breakdown of the function of each page:\n", font=main_font, bg=LIGHT_GREEN)
  help_label_1.pack()
  ...
  help_label_2 = tk.Label(window, text=help_text, font=("Arial", 11), bg=LIGHT_GREEN)
  help_label_2.pack()

# Function to create a credits page
def credits_page(window, all_windows):
  # Clears all widgets so the window can be reused
  clear_window(window)

  window.title("Bird Migration Pattern Predictor - Credits")

  # Call the function to add the menu bar to the window
  define_menu_bar(window, all_windows)

  # Create the labels that give credit to the websites that I used
  credits_label_1 = tk.Label(window, text="Here are the contributors to this project:\n​", font=main_font, bg=LIGHT_GREEN)
  credits_label_1.pack()
  credits_label_2 = tk.Label(window, text="Code and conception: William Lycett", font=main_font, bg=LIGHT_GREEN)
  credits_label_2.pack()
  credits_label_3 = tk.Label(window, text="Bird migration data: BirdTrack by the BTO", font=main_font, bg=LIGHT_GREEN, cursor="hand2")
  credits_label_3.pack()
  credits_label_3.bind("<Button-1>", bird_link)
  credits_label_4 = tk.Label(window, text="Weather data: Weather Underground", font=main_font, bg=LIGHT_GREEN, cursor="hand2")
  ...
  credits_label_5 = tk.Label(window, text=disclaimer, font=main_font, bg=LIGHT_GREEN)
